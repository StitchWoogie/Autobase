...
namespace GraphicModule
{
    public class ScriptFunctionCircle
    {
        ...
        static async Task<(int, object val)> Run_ObjectPublic(ScriptClass scriptClass, string method_name,  object[] args)
        {
            ...
            if (TotalConfig.defineMode == EnumDefineMode.MODE_RUN)
            {
                retn_value = await scriptClass.ExecuteClassName(ObjectCircle.arrayClassList, (string)args[0], method_name, args);
            }
        ...
        {
            string prename = "Circle";

            prepare.AddMethod(prename, "CircleSetType", "void", new ScriptExternalRun.AsyncDeleMethod(Run_ObjectPublic), "in:string:classname", "in:int:type");
            prepare.AddMethod(prename, "CircleSetAngle", "void", new ScriptExternalRun.AsyncDeleMethod(Run_ObjectPublic), "in:string:classname", "in:float:start_angle", "in:float:sweep_angle");
        }
    }
}
