...
{
	/// <summary>
	/// Summary description for ViewDigitalOutputMain.
	/// </summary>
	public class ViewDigitalOutputMain : System.Windows.Forms.Form	//AnalogDigitalCommonDrawClass
	{
		...
		private System.Windows.Forms.ContextMenu contextMenuDo;
		private System.Windows.Forms.MenuItem menuItem_doDetail; 
		private System.Windows.Forms.MenuItem menuItem4;
		private System.Windows.Forms.MenuItem menuItem_doValueSetting;
		...
		public ViewDigitalOutputMain(string tag)
		{
			//
		...
		private void InitializeComponent()
		{
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ViewDigitalOutputMain));
            ...
            this.contextMenuDo = new System.Windows.Forms.ContextMenu();
            this.menuItem_doDetail = new System.Windows.Forms.MenuItem();
            this.menuItem4 = new System.Windows.Forms.MenuItem();
            this.menuItem_doValueSetting = new System.Windows.Forms.MenuItem();
            ...
            // contextMenuDo
            // 
            this.contextMenuDo.MenuItems.AddRange(new System.Windows.Forms.MenuItem[] {
            this.menuItem_doDetail,
            this.menuItem4,
            this.menuItem_doValueSetting,
            this.menuItem_TagProperityModify,
            this.menuItem_TagSearch,
            this.menuItem2,
            this.menuItem1,
            this.menuItem3});
            resources.ApplyResources(this.contextMenuDo, "contextMenuDo");
            // 
            // menuItem_doDetail
            // 
            resources.ApplyResources(this.menuItem_doDetail, "menuItem_doDetail");
            this.menuItem_doDetail.Index = 0;
            this.menuItem_doDetail.Click += new System.EventHandler(this.menuItem_doDetail_Click);
            // 
            // menuItem4
            // 
            ...
            // 
            // menuItem_doValueSetting
            // 
            resources.ApplyResources(this.menuItem_doValueSetting, "menuItem_doValueSetting");
            this.menuItem_doValueSetting.Index = 2;
            this.menuItem_doValueSetting.Click += new System.EventHandler(this.menuItem_doValueSetting_Click);
            // 
            ...
            this.Name = "ViewDigitalOutputMain";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.ViewDigitalOutputMain_Load);
            this.SizeChanged += new System.EventHandler(this.ViewDigitalOutputMain_SizeChanged);
            this.Closed += new System.EventHandler(this.ViewDigitalOutputMain_Closed);
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.ViewDigitalOutputMain_KeyDown);
		...
		private void ViewDigitalOutputMain_Load(object sender, System.EventArgs e)
		{
			...
			//TotalConfig.AutoBaseListCtrlConfigLoad(ref headWidth, "BasicScreen", "DigitalOutputMain");
		...
		void listHeaderFill()
		{
            ...
            list.ConfigLoad(TotalConfig.AutoBaseIniGetConfigDirectory(), "BasicScreen", "DigitalOutputMain");
		...
		void basicElementSetting()
		{
            ...
            {
                list.contextMenu = this.contextMenuDo;
            }
		}
		...
		void setActTagMenuEnableDisable(bool flag)
		{
			...
			menuItem_doDetail.Enabled = flag;
			menuItem_doValueSetting.Enabled = flag;
		}
		...
		{
			TagDoClass dout = getSelectedDoTagClass();
			if(dout == null) 
			{
			...
			setActTagMenuEnableDisable((dout.act == 1) ? true : false);
		...
		public void callDetailWindow()
		{
			TagDoClass dout = getSelectedDoTagClass();
			if(dout == null) return;
			BasicScreenTool.callDetailWindow((TagPublicClass)dout);
		}
		...
		void setTagActiveInActive()		// Tag Active/Inactive
		{
			...
			dout.act = ( dout.act == 1 ) ? (sbyte)0 : (sbyte)1;
			...
			LibComNetServer.SendCommandToNetWorkTagMemberChanged(dout.tag, EnumTagMember.TAG_MEMBER_act, dout.act);
		}

		TagDoClass getSelectedDoTagClass()
		{
			if(list.currPos < 0 || list.currPos >= list.listHap || tagList == null) return null;
			if(tagList.Length <= list.currPos) return null;

			return TagLib.GetStructDO(tagList[list.currPos]);
		}
		...
		void callValueChangeWindows()
		{
			TagDoClass dout = getSelectedDoTagClass();
			if(dout == null) return;
			if(BasicScreenTool.callValueChangeWindows(this, (TagPublicClass)dout))
				list.oneLineInvalidate(list.currPos);
		...
		private void menuItem_doDetail_Click(object sender, System.EventArgs e)
		{
			callDetailWindow();
		}

		private void menuItem_doValueSetting_Click(object sender, System.EventArgs e)
		{
		...
		private void ViewDigitalOutputMain_SizeChanged(object sender, System.EventArgs e)
		{
		...
		{
			if(tagevent.enumTagType != EnumTagType.DO) return;

			TagDoClass	dout;

            for (int i = list.startPos; i < list.listHap && i <= list.startPos + list.pageLineCount; i++) 
			{
				dout = TagLib.GetStructDO(tagList[i]);
				if(dout == null || dout.tag != tagevent.tag) continue;
		...
		{
			if(tp.enumTagType != EnumTagType.DO) return;
			TagDoClass	dout;
			for(int i = 0; i < list.listHap; i++) 
			{
				dout = TagLib.GetStructDO(tagList[i]);
				if(dout == null || dout != (TagDoClass)tp) continue;
				list.oneLineInvalidate(i);
		...
		void oneLineDraw(Graphics g, TagDoClass dout, int i, int x, int y, int xGap)
		{
            Color color_etc = (dout.act == 1) ? SharedData.colorTotal.TEXT : SharedData.colorTotal.INACTIVE;
            ...
            for (int c = 0; c < list.header.Count; c++)
            {
                ...
                {
                    Color color = (dout.act == 1) ? SharedData.colorTotal.TAG : SharedData.colorTotal.INACTIVE;
                    DrawClass.WinDrawText(g, x + xGap, y + 1, head.width - xGap * 2, (int)list.fontY, dout.tag, color, list.backColor, list.font, format);
                }
                else if (head.nID == (int)TagListViewColumn.EnumTagListColumn.Description)
                {
                    Color color = (dout.act == 1) ? SharedData.colorTotal.DESCRIPTION : SharedData.colorTotal.INACTIVE;
                    DrawClass.WinDrawText(g, x + xGap, y + 1, head.width - xGap * 2, (int)list.fontY, dout.description, color, list.backColor, list.font, format);
                }
                else if (head.nID == (int)TagListViewColumn.EnumTagListColumn.Value)
                {
                    BasicScreenTool.DoCurrValAndStatusButtonDraw(g, dout, x, y, head.width, list.fontY, list.font);
                }
                else if (head.nID == (int)TagListViewColumn.EnumTagListColumn.Port)
                {
                    DrawClass.WinDrawText(g, x + xGap, y + 1, head.width - xGap * 2, (int)list.fontY, dout.port.ToString(), color_etc, list.backColor, list.font, head.format);
                }
                else if (head.nID == (int)TagListViewColumn.EnumTagListColumn.Address)
                {
                    DrawClass.WinDrawText(g, x + xGap, y + 1, head.width - xGap * 2, (int)list.fontY, dout.address.ToString("X"), color_etc, list.backColor, list.font, head.format);
                }
			...
			color = (dout.act == 1) ? SharedData.colorTotal.TEXT : SharedData.colorTotal.INACTIVE;
			...
			color = (dout.act == 1) ? SharedData.colorTotal.TAG : SharedData.colorTotal.INACTIVE;
			DrawClass.WinDrawText(g, x+xGap, y+1, head.width-xGap*2, (int)list.fontY, dout.tag, color, list.backColor, list.font, format);
			
			x += head.width;
			head = (ControlListViewHeader)list.header[2];
			color = (dout.act == 1) ? SharedData.colorTotal.DESCRIPTION : SharedData.colorTotal.INACTIVE;
			DrawClass.WinDrawText(g, x+xGap, y+1, head.width-xGap*2, (int)list.fontY, dout.description, color, list.backColor, list.font, format);
		...
		void DrawData(Graphics g, Rectangle r)
		{
			...
			for(pos = list.startPos; pos < endPos; pos++, y += list.fontY) 
			{
				...
				x = list.startX;
				oneLineDraw(g, dout, pos+1, x, y, xGap);
			}
		}
		...
		private void timer1_Tick(object sender, System.EventArgs e)
		{
			...
			for(int i = list.startPos; i < list.listHap; i++) 
			{
				...
				dout = TagLib.GetStructDO(tagList[i]);
				if(dout == null || dout.act == 0) continue;
				dout.NeedDataCurr = true;
			}
		}

		private void ViewDigitalOutputMain_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{
			switch(e.KeyCode)
			{
		...
		private void ViewDigitalOutputMain_Closed(object sender, System.EventArgs e)
		{
			SharedViewMain.EventListTagChanged -= new SharedViewMain.OnEventTagChanged(OnEventTagChanged);
			SharedViewMain.EventListMainFontChanged -= new SharedViewMain.OnEventMainFontChanged(OnMainFontChanged);
			SharedViewMain.EventListColorChanged -= new SharedViewMain.DelegatePublic(OnColorChanged);
            ...
            list.ConfigSave(TotalConfig.AutoBaseIniGetConfigDirectory(), "BasicScreen", "DigitalOutputMain");
			...
			if(bChange) TotalConfig.AutoBaseListCtrlConfigSave(ref headWidth, "BasicScreen", "DigitalOutputMain");
             */
