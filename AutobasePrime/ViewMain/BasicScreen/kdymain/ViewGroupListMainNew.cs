...
{
	/// <summary>
	/// Summary description for ViewGroupTagListMainNew.
	/// </summary>ViewGroupTagListMainNew
	public class ViewGroupTagListMainNew : System.Windows.Forms.Form
	{
		...
		private System.Windows.Forms.ContextMenu contextMenuGroupTree;
		private System.Windows.Forms.MenuItem menuItem_GroupProperty;
		private System.Windows.Forms.MenuItem menuItem6;
		private System.Windows.Forms.MenuItem menuItem16;
		string					selectedNodeString = "Local";
        private MenuItem menuItem7; //트리선택 위치를 기억하기 위해 , 2번 이상 그려지는 것을 방자

        TagListViewColumn tlvc = new TagListViewColumn();

		public ViewGroupTagListMainNew()
		{
			//
		...
		private void InitializeComponent()
		{
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ViewGroupTagListMainNew));
            ...
            this.contextMenuGroupTree = new System.Windows.Forms.ContextMenu();
            this.menuItem_GroupProperty = new System.Windows.Forms.MenuItem();
            ...
            // 
            // contextMenuGroupTree
            // 
            this.contextMenuGroupTree.MenuItems.AddRange(new System.Windows.Forms.MenuItem[] {
            this.menuItem_GroupProperty,
            this.menuItem6,
            this.menuItem16});
            // 
            // menuItem_GroupProperty
            // 
            this.menuItem_GroupProperty.Index = 0;
            resources.ApplyResources(this.menuItem_GroupProperty, "menuItem_GroupProperty");
            this.menuItem_GroupProperty.Click += new System.EventHandler(this.menuItem_GroupProperty_Click);
            // 
            // menuItem6
            // 
            ...
            // menuItem5
            // 
            this.menuItem5.Index = 26;
            resources.ApplyResources(this.menuItem5, "menuItem5");
            // 
            // menuItem3
            // 
            this.menuItem3.Index = 27;
            resources.ApplyResources(this.menuItem3, "menuItem3");
            // 
            // ViewGroupTagListMainNew
            // 
            ...
            this.Name = "ViewGroupTagListMainNew";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.ViewGroupTagListMainNew_Load);
            this.SizeChanged += new System.EventHandler(this.ViewGroupTagListMainNew_SizeChanged);
            this.Closed += new System.EventHandler(this.ViewGroupTagListMainNew_Closed);
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.ViewGroupTagListMainNew_KeyDown);
		...
		private void ViewGroupTagListMainNew_Load(object sender, System.EventArgs e)
		{
			...
			//TotalConfig.AutoBaseListCtrlConfigLoad(ref headWidth, "BasicScreen", "GroupTagList");
			
			bLoading = true;			
			fillStartTreeAndList();
            ...
            // 선택을 하지 않으니 중간에 아무거나 선택된다. 2016-4-28 추가
		...
		{
			this.treeView1.Font = ConfigViewMain.fontMain;
			this.treeView1.ItemHeight = this.treeView1.Font.Height;
			this.treeView1.BackColor = SharedData.colorTotal.BACK;
			this.treeView1.ForeColor = SharedData.colorTotal.TEXT;
		...
		void listHeaderFill()
		{
            ...
            list.ConfigLoad(TotalConfig.AutoBaseIniGetConfigDirectory(), "BasicScreen", "GroupTagList");
		...
		void fillStartTreeAndList()
		{			
			this.treeView1.Nodes.Clear();
			TagLib.FillGroupTree(this.treeView1);
		...
		void fillListDataAll()
		{
			...
			list.listHap = 0;
			if(bAllTagDisplay && path == "") 
			{
				bTagList = true;
				tagList = TagLib.tagListAll;
				if(tagList != null) 
				{
					list.listHap = tagList.Length;
				}
			}
			else 
			{
				bTagList = false;
				gr = TagLib.GetGroupArray(TagLib.groupRoot, path);
				list.listHap = gr.arrayTag.Count;
			}
			list.listItemChanged();
						
			setDetailButtonEnableDisable();
			selectedNodeString = getSelectedNodeString();
		}

		string getSelectedNodeString()
		{
			if(treeView1.SelectedNode == null || treeView1.SelectedNode.Nodes == null) return "Local";// 첫 트리를( Local ) 그린다
		...
		private void treeView1_AfterSelect(object sender, System.Windows.Forms.TreeViewEventArgs e)
		{
			...
			string		str = getSelectedNodeString();
			if(str == null) return;
			if(selectedNodeString == str) return;
		...
		void setTagActiveInActive()		// Tag Active/Inactive
		{
			...
			LibComNetServer.SendCommandToNetWorkTagMemberChanged(tp.tag, EnumTagMember.TAG_MEMBER_act, tp.act);
		}
		...
		void OnTagPropertyChanged(TagPublicClass tp)
		{
			...
			for(int i = 0; i < list.listHap; i++) 
			{
				...
				{
					if(tagList == null) return;
					tp2 = TagLib.GetStructPublic(tagList[i]);
				}
				else 
					tp2 = (TagPublicClass)gr.arrayTag[i];
		...
		void oneLineDraw(Graphics g, TagPublicClass	tp, int i, int x, int y, int xGap)
		{
            ...
            for (int c = 0; c < list.header.Count; c++)
            {
                ...
                else if (head.nID == (int)TagListViewColumn.EnumTagListColumn.Tag)
                {
                    ...
                    if (bTagList) DrawClass.WinDrawText(g, x + xGap, y + 1, head.width - xGap * 2, (int)list.fontY, tp.tag, color, list.backColor, list.font, format);
                    else DrawClass.WinDrawText(g, x + xGap, y + 1, head.width - xGap * 2, (int)list.fontY, tp.name, color, list.backColor, list.font, format);
                }
			...
			color = (tp.act == 1) ? SharedData.colorTotal.TAG : SharedData.colorTotal.INACTIVE;
			if(bTagList) DrawClass.WinDrawText(g, x+xGap, y+1, head.width-xGap*2, (int)list.fontY, tp.tag, color, list.backColor, list.font, format);
			else	     DrawClass.WinDrawText(g, x+xGap, y+1, head.width-xGap*2, (int)list.fontY, tp.name, color, list.backColor, list.font, format);

			x += head.width;
			head = (ControlListViewHeader)list.header[2];
			color = (tp.act == 1) ? SharedData.colorTotal.DESCRIPTION : SharedData.colorTotal.INACTIVE;
			DrawClass.WinDrawText(g, x+xGap, y+1, head.width-xGap*2, (int)list.fontY, tp.description, color, list.backColor, list.font, format);
        ...
        bool bNeedInvalidateOnTimer = false;
		...
		TagPublicClass getSelectedTagPublicClass()
		{
			...
			if(bAllTagDisplay && GetGroupName() == "") 
			{
				if(tagList == null) return null;
				if(tagList.Length <= list.currPos) return null;
				return TagLib.GetStructPublic(tagList[list.currPos]);
			}
			else 
			{
				if(gr.arrayTag.Count <= list.currPos) return null;
				return (TagPublicClass)gr.arrayTag[list.currPos];
			}
		}

		TreeNode getNextNodeFromString(string text)
		{
			...
			for(int i = 0; i < node.Nodes.Count; i++) 
			{
				...
				if(text == nextNode.Text) return nextNode;
			}
			return null;
		}

		void changeGroupShow(TagPublicClass tp)
		{
			TreeNode	node = getNextNodeFromString(tp.name);
			if(node == null) return;
		...
		public void callDetailWindow()
		{
            ...
            else if (tp.enumTagType == EnumTagType.GR) changeGroupShow(tp);
		...
		void callValueChangeWindows()
		{
			...
			{
				//ModifyOneListItemVal(tp, list.currPos);
				list.oneLineInvalidate(list.currPos);
			}
		}
		...
		void CallSelectTagPropertyWindows()
		{
			TagPublicClass tp = getSelectedTagPublicClass();
			if(tp == null) return;
			if(DialogTag.TagEditor.Editor.ByViewMain(tp) == false) return;
		...
		private void menuItem_GroupProperty_Click(object sender, System.EventArgs e)
		{
			string		path = GetGroupName();
			if(path == null || path == "") 
			{
				ViewGroupTagListSettingDlg dialog = new ViewGroupTagListSettingDlg();
				dialog.checkBox_AllTagDisplay.Checked = this.bAllTagDisplay;
                dialog.StartPosition = FormStartPosition.CenterParent;
				dialog.ShowDialog(this);
				if(dialog.DialogResult == DialogResult.OK && this.bAllTagDisplay != dialog.checkBox_AllTagDisplay.Checked) 
				{
					this.bAllTagDisplay = dialog.checkBox_AllTagDisplay.Checked;
					fillListDataAll();
			...
			TagPublicClass			tp = TagLib.GetStructPublic(path, ref nTagPos);

			if(tp == null) return;
			DialogTag.TagEditor.Editor.ByViewMain(tp);
		}
		...
		private void treeView1_MouseDown(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			...
			if(path == "") 
			{
				if(Tools.IsLangKorean()) menuItem_GroupProperty.Text = "전체태그 보기 설정(&G)";
                else if (Tools.IsLangJapanese()) menuItem_GroupProperty.Text = "全タグ表示設定(&G)";
				else if(Tools.IsLangChinese()) menuItem_GroupProperty.Text = "设置查看全体标记(&G)";
				else					 menuItem_GroupProperty.Text = "&Tags View Settings";
			}
			else 
			{
				if(Tools.IsLangKorean()) menuItem_GroupProperty.Text = "그룹속성 설정(&G)";
				else if(Tools.IsLangJapanese()) menuItem_GroupProperty.Text = "グループのプロパティ(&G)";
				else if(Tools.IsLangChinese()) menuItem_GroupProperty.Text = "设置组属性(&G)";
				else					 menuItem_GroupProperty.Text = "&Group Properties";
			}
			this.contextMenuGroupTree.Show(this.treeView1, p);
		}
		...
		void groupListTagSearchNameOrPos()
		{
			...
			ViewTagNamePosSearchDlg dialog = new ViewTagNamePosSearchDlg(list.currPos, list.listHap);
			if(bTagList) dialog.tagList = tagList;
			else		 dialog.gr = gr;
			dialog.bTagList = bTagList;
            dialog.StartPosition = FormStartPosition.CenterParent;
			dialog.ShowDialog(this);
			if(dialog.bTagChanged && list.currPos != dialog.currPos)		// 값이 변경되었다.
			{
		...
		private void ViewGroupTagListMainNew_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{
			switch(e.KeyCode)
			{
		...
		private void ViewGroupTagListMainNew_SizeChanged(object sender, System.EventArgs e)
		{
			this.panel1.Height = this.toolBar1.Height;
		}

		private void ViewGroupTagListMainNew_Closed(object sender, System.EventArgs e)
		{
			SharedViewMain.EventListTagChanged -= new SharedViewMain.OnEventTagChanged(OnEventTagChanged);
			SharedViewMain.EventListMainFontChanged -= new SharedViewMain.OnEventMainFontChanged(OnMainFontChanged);
			SharedViewMain.EventListColorChanged -= new SharedViewMain.DelegatePublic(OnColorChanged);
            ...
            list.ConfigSave(TotalConfig.AutoBaseIniGetConfigDirectory(), "BasicScreen", "GroupTagList");
			...
			if(bChange) TotalConfig.AutoBaseListCtrlConfigSave(ref headWidth, "BasicScreen", "GroupTagList");*/
