	...
	public class GZipInputStream : InflaterInputStream 
	{
		...
		void ReadFooter() 
		{
			...
			while (needed > 0) {
				...
				if (count <= 0) {
                    throw new System.Exception("Early EOF baseInputStream GZIP footer");
				}
			...
			if (crcval != (int) crc.Value) {
                throw new System.Exception("GZIP crc sum mismatch, theirs \"" + crcval + "\" and ours \"" + (int)crc.Value);
			}
			
			int total = (footer[4] & 0xff) | ((footer[5] & 0xff) << 8) | ((footer[6] & 0xff) << 16) | (footer[7] << 24);
			if (total != inf.TotalOut) {
                throw new System.Exception("Number of bytes mismatch");
			}
