...
namespace ExportGlofaCnetTestRandomRead
{
	/// <summary>
	...
	public class ClassExportGate2 : ExportLib.ClassExportLib2
	{
		...
		{
			return "Glofa Cnet Test Random Read";
		}
		...
		void ExcuteReadCommand()
		{
			if(commRecvBuf[4] == 'S' && commRecvBuf[5] == 'B') 
			{
				...
				for(int i = 0; i < read_size; i++) 
				{
					if(commRecvBuf[10] == 'X') 
					{
						imsi = String.Format("{0:X02}", (byte)rand.Next());
						data_size = 2;
					}
					else if(commRecvBuf[10] == 'W') 
					{
						imsi = String.Format("{0:X04}", (int)rand.Next());
						data_size = 4;
					}
					else if(commRecvBuf[10] == 'D') 
					{
						imsi = String.Format("{0:X08}", (int)rand.Next());
						data_size = 8;
					}
					else 
					{
