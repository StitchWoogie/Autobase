...
namespace OpcXml.Da10
{
	...
	{		
		...
		{
			return absoluteTime.Subtract(DateTime.Now);
		}
		...
		public static Opc.ResultID GetResultID(XmlQualifiedName input)
		{
			...
			if (input == OpcXml.Da.Error.E_UNKNOWN_ITEM_NAME)        return Opc.ResultID.E_UNKNOWN_ITEM_NAME;
			if (input == OpcXml.Da.Error.E_INVALID_ITEM_NAME)        return Opc.ResultID.E_INVALID_ITEM_NAME;
			if (input == OpcXml.Da.Error.E_UNKNOWN_ITEM_PATH)        return Opc.ResultID.E_UNKNOWN_ITEM_PATH;
			if (input == OpcXml.Da.Error.E_INVALID_ITEM_PATH)        return Opc.ResultID.E_INVALID_ITEM_PATH;
			if (input == OpcXml.Da.Error.E_BADTYPE)                  return Opc.ResultID.E_BADTYPE;
			if (input == OpcXml.Da.Error.E_RANGE)                    return Opc.ResultID.E_RANGE;
			if (input == OpcXml.Da.Error.E_READONLY)                 return Opc.ResultID.E_READONLY;
			if (input == OpcXml.Da.Error.E_WRITEONLY)                return Opc.ResultID.E_WRITEONLY;
			if (input == OpcXml.Da.Error.E_NOTSUPPORTED)             return Opc.ResultID.E_NO_WRITEQT;
			if (input == OpcXml.Da.Error.E_INVALID_PID)              return Opc.ResultID.E_INVALID_PID;
			if (input == OpcXml.Da.Error.S_FALSE)                    return Opc.ResultID.S_FALSE;
			if (input == OpcXml.Da.Error.S_CLAMP)                    return Opc.ResultID.S_CLAMP;
			if (input == OpcXml.Da.Error.S_UNSUPPORTEDRATE)          return Opc.ResultID.S_UNSUPPORTEDRATE;
			if (input == OpcXml.Da.Error.S_DATAQUEUEOVERFLOW)        return Opc.ResultID.S_DATAQUEUEOVERFLOW;
			if (input == OpcXml.Da.Error.E_TYPE_CHANGED)             return Opc.ResultID.E_TYPE_CHANGED;
			if (input == OpcXml.Da.Error.E_FILTER_DUPLICATE)         return Opc.ResultID.E_FILTER_DUPLICATE;
			if (input == OpcXml.Da.Error.E_FILTER_INVALID)           return Opc.ResultID.E_FILTER_INVALID;
			if (input == OpcXml.Da.Error.E_FILTER_ERROR)             return Opc.ResultID.E_FILTER_ERROR;
			if (input == OpcXml.Da.Error.S_FILTER_NO_DATA)           return Opc.ResultID.S_FILTER_NO_DATA;
		...
		public static XmlQualifiedName GetResultID(Opc.ResultID input)
		{
			...
			if (input == Opc.ResultID.S_DATAQUEUEOVERFLOW) return OpcXml.Da.Error.S_DATAQUEUEOVERFLOW; 
			if (input == Opc.ResultID.S_UNSUPPORTEDRATE)   return OpcXml.Da.Error.S_UNSUPPORTEDRATE; 
			if (input == Opc.ResultID.S_CLAMP)             return OpcXml.Da.Error.S_CLAMP; 
			...
			if (input == Opc.ResultID.E_INVALIDHANDLE)     return Opc.ResultID.E_INVALIDHANDLE.Name; 
			if (input == Opc.ResultID.E_UNKNOWN_ITEM_NAME) return OpcXml.Da.Error.E_UNKNOWN_ITEM_NAME; 
			if (input == Opc.ResultID.E_INVALID_ITEM_NAME) return OpcXml.Da.Error.E_INVALID_ITEM_NAME; 
			if (input == Opc.ResultID.E_UNKNOWN_ITEM_PATH) return OpcXml.Da.Error.E_UNKNOWN_ITEM_PATH; 
			if (input == Opc.ResultID.E_INVALID_ITEM_PATH) return OpcXml.Da.Error.E_INVALID_ITEM_PATH; 
			if (input == Opc.ResultID.E_INVALID_PID)       return OpcXml.Da.Error.E_INVALID_PID; 
			if (input == Opc.ResultID.E_READONLY)          return OpcXml.Da.Error.E_READONLY; 
			if (input == Opc.ResultID.E_WRITEONLY)         return OpcXml.Da.Error.E_WRITEONLY; 
			if (input == Opc.ResultID.E_WRITEONLY)         return OpcXml.Da.Error.E_WRITEONLY; 
			if (input == Opc.ResultID.E_BADTYPE)           return OpcXml.Da.Error.E_BADTYPE; 
			if (input == Opc.ResultID.E_RANGE)             return OpcXml.Da.Error.E_RANGE; 
			if (input == Opc.ResultID.E_NO_WRITEQT)        return OpcXml.Da.Error.E_NOTSUPPORTED; 
			if (input == Opc.ResultID.E_NO_ITEM_DEADBAND)  return Opc.ResultID.E_NO_ITEM_DEADBAND.Name; 
			if (input == Opc.ResultID.E_NO_ITEM_SAMPLING)  return Opc.ResultID.E_NO_ITEM_SAMPLING.Name; 
			if (input == Opc.ResultID.E_NO_ITEM_BUFFERING) return Opc.ResultID.E_NO_ITEM_BUFFERING.Name; 
			if (input == Opc.ResultID.E_TYPE_CHANGED)      return OpcXml.Da.Error.E_TYPE_CHANGED;
			if (input == Opc.ResultID.E_FILTER_DUPLICATE)  return OpcXml.Da.Error.E_FILTER_DUPLICATE;
			if (input == Opc.ResultID.E_FILTER_INVALID)    return OpcXml.Da.Error.E_FILTER_INVALID;
			if (input == Opc.ResultID.E_FILTER_ERROR)      return OpcXml.Da.Error.E_FILTER_ERROR;
			if (input == Opc.ResultID.S_FILTER_NO_DATA)    return OpcXml.Da.Error.S_FILTER_NO_DATA;
		...
		public static PropertyID GetPropertyID(XmlQualifiedName input)
		{
			...
			try
			{
				return new PropertyID(input.Name, System.Convert.ToInt32(input.Name), input.Namespace);
			}
			catch
			{
		...
		internal static XmlQualifiedName GetType(System.Type input)
		{
			...
			if (input == typeof(sbyte[]))    return OpcXml.Da.Type.BINARY;
