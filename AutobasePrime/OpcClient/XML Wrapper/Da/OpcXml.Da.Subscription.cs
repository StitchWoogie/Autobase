...
namespace OpcXml.Da
{
	...
	{				
		...
		{ 
			throw new NotImplementedException("BeginWrite");
		}
		...
		{	
			lock (this)
			{
				if (m_callback != null)
				{
					m_callback(m_state.ClientHandle, (ItemValueResult[])items.ToArray(typeof(ItemValueResult)));
				}
			}
		}
		...
		private void  OnPollCompleted(IAsyncResult handle)
		{
			...
			{
				// this line makes it possible to set a breakpoint.
