	...
	{	
			...
			lock (this)
			{
				...
				try
				{
					...
					((IOPCServer)m_server).GetErrorString(
						resultID.Code, 
						OpcCom.Da.Interop.GetLocale(locale),
						out errorText);
			...
			lock (this)
			{
				...
				for (int ii = 0; ii < results.Length; ii++)
				{
					...
					if (results[ii].Value != null && items[ii].ReqType != null)
					{
						try
						{
							results[ii].Value = Opc.Convert.ChangeType(values[ii], items[ii].ReqType);
						}
						catch (Exception e)
						{
			...
			lock (this)
			{
				...
				try
				{
					((IOPCServer)m_server).AddGroup(
						...
						OpcCom.Da.Interop.GetLocale(result.Locale),
						out serverHandle,
		...
		{		
			...
			lock (this)
			{
				...
				{
					foreach (ItemProperty property in resultList)
					{
