...
namespace OpcCom.Da
{
	...
	{	
		...
		{
			add    {lock (this){ Advise(); m_callback.DataChanged += value; }}
		...
		public SubscriptionState GetState() 
		{
			...
			{ 
				...
				try
				{
					...
					state.Locale       = OpcCom.Da.Interop.GetLocale(localeID);
		...
		public Opc.Da.SubscriptionState ModifyState(int masks, Opc.Da.SubscriptionState state)
		{
			...
			lock (this)
			{
				...
				int localeID = ((masks & (int)StateMask.Locale) != 0)?OpcCom.Da.Interop.GetLocale(state.Locale):0;
			...
			lock (this)
			{
				...
				for (int ii = 0; ii < itemIDs.Length; ii++)
				{
					...
					if (results[ii].ResultID.Succeeded())
					{
						lock (m_items)
						{
							m_items[results[ii].ClientHandle] = null;
						}
					}
				}
		...
		/// <param name="clientHandle">A client assigned identifier for the request.</param>
		/// <param name="callback">The function to invoke when the request completes.</param>
		/// <returns>An identifier for the request (may be used to cancel the request).</returns>
		public Opc.Da.Request BeginRead(Item[] items, object clientHandle, ReadCompleteEventHandler callback)
		{ 
			if (items == null)    throw new ArgumentNullException("items");
			...
			if (items.Length == 0)
			{
				callback(clientHandle, new ItemValueResult[0]);
				return null;
			}
			...
			lock (this)
			{
				...
				OpcCom.Da.Request request = new OpcCom.Da.Request(
					this, 
					clientHandle, 
					m_filters,
					m_counter++,
				...
				m_callback.BeginRequest(request);
				...
				try
				{
					results = BeginRead(itemIDs, items, request.RequestID, out cancelID);
				}
				catch (Exception e)
				{
					m_callback.BeginRequest(request);
				...
				lock (request)
				{
					...
					if (request.BeginRead(cancelID, results))
					{
						m_callback.EndRequest(request);
						return null;
					}
				}
		...
		/// <param name="callback">The function to invoke when the request completes.</param>
		/// <returns>An identifier for the request (may be used to cancel the request).</returns>
		public Opc.Da.Request BeginWrite(ItemValue[] items, object clientHandle, WriteCompleteEventHandler callback)
		{ 
			if (items == null)    throw new ArgumentNullException("items");
			if (callback == null) throw new ArgumentNullException("callback");
			...
			if (items.Length == 0)
			{
				callback(clientHandle, new IdentifiedResult[0]);
				return null;
			}
			...
			lock (this)
			{
				...
				try
				{
					results = BeginWrite(itemIDs, items, request.RequestID, out cancelID);
				}
				catch (Exception e)
				{
					m_callback.BeginRequest(request);
				...
				{
					// check if all results have already arrived - this invokes the callback if this is the case.
					if (request.BeginWrite(cancelID, results))
					{
						m_callback.EndRequest(request);
						return null;
					}
		...
		public void Cancel(Opc.Da.Request request, CancelCompleteEventHandler callback) 
		{ 
		...
		protected virtual ItemValueResult[] Read(ItemIdentifier[] itemIDs, Item[] items)
		{
			try
			{
				...
				for (int ii = 0; ii < itemIDs.Length; ii++)
				{
					...
					if (errors[ii] == ResultIDs.E_BADRIGHTS) { results[ii].ResultID = new ResultID(ResultID.E_WRITEONLY, ResultIDs.E_BADRIGHTS); }
				}
		...
		private void SetReqTypes(ItemResult[] items)
		{
			...
			try
			{
				...
				for (int ii = 0; ii < errors.Length; ii++)
				{
					...
					{
						ItemResult item     = (ItemResult)changedItems[ii];
						item.ResultID       = ResultID.E_BADTYPE;
		...
		private class ItemTable
		{
			...
			public ItemResult[] CreateItems(Item[] items)
			{
				...
				{		
					...
					{
						results[ii].ResultID       = ResultID.E_INVALIDHANDLE;
						results[ii].DiagnosticInfo = null;
						continue;
					}
		...
		private class Callback : OpcRcw.Da.IOPCDataCallback
		{
				...
				try
				{
					...
					lock (this)
					{
						...
						for (int ii = 0; ii < results.Length; ii++)
						{
							...
							// convert COM code to unified DA code.
							if (pErrors[ii] == ResultIDs.E_BADRIGHTS) { results[ii].ResultID = new ResultID(ResultID.E_READONLY, ResultIDs.E_BADRIGHTS); }
						}
					}
				...
				int[]                pErrors)
			{
				...
				for (int ii = 0; ii < values.Length; ii++)
				{
					...
					// convert COM code to unified DA code.
					if (pErrors[ii] == ResultIDs.E_BADRIGHTS) { values[ii].ResultID = new ResultID(ResultID.E_WRITEONLY, ResultIDs.E_BADRIGHTS); }
				}
	...
	{	
		...
		public bool BeginRead(int cancelID, IdentifiedResult[] results)
		{
			...
			// construct empty item value result list.			
			values = new ItemValueResult[results.Length];

			for (int ii = 0; ii < results.Length; ii++)
			{
				values[ii] = new ItemValueResult(results[ii]);

				values[ii].Value              = null;
				values[ii].Quality            = Quality.Bad;
				values[ii].QualitySpecified   = false;
				values[ii].Timestamp          = DateTime.MinValue;
				values[ii].TimestampSpecified = false;
				values[ii].ResultID           = results[ii].ResultID;
				values[ii].DiagnosticInfo     = results[ii].DiagnosticInfo;

				if ((Filters & (int)ResultFilter.ItemName) == 0)     values[ii].ItemName = null; 
				if ((Filters & (int)ResultFilter.ItemPath) == 0)     values[ii].ItemPath = null; 
				if ((Filters & (int)ResultFilter.ClientHandle) == 0) values[ii].ClientHandle = null; 
			}

			// invoke callback.
			((ReadCompleteEventHandler)Callback)(Handle, values);
		...
		/// Completes a read request by processing the values and invoking the callback.
		/// </summary>
		public void EndRequest(ItemValueResult[] values)
		{
			...
			// construct composite item value result list.			
			ItemValueResult[] results = new ItemValueResult[InitialResults.Length];

			for (int ii = 0; ii < results.Length; ii++)
			{
				// create item value object.
				results[ii] = new ItemValueResult(InitialResults[ii]);
				...
				// check for items that failed when the request was begun.
				if (((IdentifiedResult[])InitialResults)[ii].ResultID.Failed())
				{
					results[ii].ResultID       = ((IdentifiedResult[])InitialResults)[ii].ResultID;
					results[ii].DiagnosticInfo = ((IdentifiedResult[])InitialResults)[ii].DiagnosticInfo;
				}
			}
			...
			for (int ii = 0; ii < values.Length; ii++)
			{
				while (index < results.Length)
				{
					...
					if (values[ii].ClientHandle.Equals(results[index].ServerHandle))
					{
						// swap the handles for return to the client.
						values[ii].ServerHandle = results[index].ServerHandle;
						values[ii].ClientHandle = results[index].ClientHandle;

						results[index++] = values[ii];
						break;
					}
			...
			// invoke callback.
			if (Callback != null && Callback.GetType() == typeof(ReadCompleteEventHandler))
			{
