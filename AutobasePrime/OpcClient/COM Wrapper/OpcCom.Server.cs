...
namespace OpcCom
{
	...
	{	
		...
		{
			add    {lock (this){ Advise(); m_callback.ServerShutdown += value; }}
			remove {lock (this){ m_callback.ServerShutdown -= value; Unadvise(); }}
		}
		...
		public virtual string GetLocale()
		{
			lock (this)
			{
				...
				try
				{
					...
					return OpcCom.Da.Interop.GetLocale(localeID);
				}
				catch (Exception e)
				{
		...
		public virtual string SetLocale(string locale)
		{
			lock (this)
			{
				...
				int lcid = OpcCom.Da.Interop.GetLocale(locale);
			...
			lock (this)
			{
				...
				try
				{
					...
					if (localeIDs != null)
					{
						...
						{
							locales.Add(OpcCom.Da.Interop.GetLocale(localeID));
						}
