	...
	{	
			...
			ref DateTime[]        timestamps)
		{
			...
			for (int ii = 0; ii < items.Length; ii++)
			{
				foreach (ItemValue value in items[ii])
				{
					...
					// using bitwise operators in C# causes unexpected problems - do it the hard way.
					byte[] quality          = BitConverter.GetBytes((short)value.Quality.GetCode());
					byte[] historianQuality = BitConverter.GetBytes((int)value.HistorianQuality);
					...
					qualities[index] = BitConverter.ToInt32(bytes, 0);
		...
		private IdentifiedResult[] GetIdentifiedResults(ResultCollection[] results)
		{
			...
			for (int ii = 0; ii < results.Length; ii++)
			{
				...
				{
					items[ii].ResultID = ResultID.S_NODATA;
					continue;
				}
