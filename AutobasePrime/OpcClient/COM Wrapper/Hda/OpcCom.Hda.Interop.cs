...
namespace OpcCom.Hda
{
	...
	{		
		...
		/// </summary>
		internal static OpcRcw.Hda.OPCHDA_FILETIME Convert(System.Runtime.InteropServices.ComTypes.FILETIME input)
		{
		...
		/// </summary>
        internal static System.Runtime.InteropServices.ComTypes.FILETIME Convert(OpcRcw.Hda.OPCHDA_FILETIME input)
		{
		...
		internal static ItemValueCollection GetItemValueCollection(OpcRcw.Hda.OPCHDA_ITEM input, bool deallocate)
		{
			...
			for (int ii = 0; ii < input.dwCount; ii++)
			{
				...
				value.HistorianQuality = (Opc.Hda.Quality)Enum.ToObject(typeof(Opc.Hda.Quality), (int)(qualities[ii] & 0xFFFF0000));
		...
		internal static ModifiedValueCollection GetModifiedValueCollection(OpcRcw.Hda.OPCHDA_MODIFIEDITEM input, bool deallocate)
		{
			...
			for (int ii = 0; ii < input.dwCount; ii++)
			{
				...
				value.EditType         = (Opc.Hda.EditType)Enum.ToObject(typeof(Opc.Hda.EditType), editTypes[ii]);
				value.User             = users[ii];
