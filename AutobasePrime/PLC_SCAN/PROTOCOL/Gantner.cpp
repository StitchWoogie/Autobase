...
int PlcScanReadGantner(LOCAL_PORT_STRUCT *pt, int pos)
{
	WORD crc = 0;
	...
	if(pt->bReadingFlag == OFF) {
		...
		crc = GetCRC_16_15_13_1(pt->commSendBuf, 6);
	...
	while(1) {
		...
		if(pt->commCountCurr >= pt->commCountNeed) {
			...
			if(crc != pt->commRecvBuf[pt->commCountNeed-2]+pt->commRecvBuf[pt->commCountNeed-1]*256u) {
...
static int GantnerWriteWord(LOCAL_PORT_STRUCT *pt, int station, WORD address, WORD value)
{
	WORD crc = 0;
	int  	 count;
...
int PlcScanWriteBitGantner(LOCAL_PORT_STRUCT *pt, int station, WORD address, WORD flag)
{
	return GantnerWriteWord(pt, station, address/16, flag ? 0xFFFF : 0x0000);
}
...
int PlcScanWriteWordGantner(LOCAL_PORT_STRUCT *pt, int station, WORD address, WORD value)
{
	return GantnerWriteWord(pt, station, address, value);
}
