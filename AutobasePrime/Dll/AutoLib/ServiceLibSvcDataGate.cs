...
namespace AutoLib
{
    public class ServiceLibSvcDataGate
    {
        ...
        /*
        public static System.ServiceModel.BasicHttpBinding MakeDefaultBinding()
        {
        ...
        {
            // if (saveDataGate != null) return saveDataGate;

            // 클라이언트가 Faulted 상태라면 무조건 새로 생성 , 20250417 PSU 수정, 경보웹서버 오류 시 IIS를 재시작해야지만 연결되는 점 개선.
            if (saveDataGate != null)
            {
                if (saveDataGate.State == CommunicationState.Faulted ||
                  saveDataGate.State == CommunicationState.Closed)
                {
                    try
                    {
                        saveDataGate.Abort();
        ...
        public int Command(string command, params object[] param)
        {
            ...
            try
            {
                //retn = service.CommonMethod( ListToByte(send), out recv);
                retn = 1;
                recv = null;
            }
