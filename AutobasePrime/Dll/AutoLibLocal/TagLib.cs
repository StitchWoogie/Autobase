...
namespace AutoLibLocal
{
	...
	{
		public string tag;
		public int[] tag_pos_save;          // 일반적으로 사용하지 말고 태그를 파일에 저장 시 pos[] 를 저장해 놓으면 다시 로딩 시 빠르게 태그를 찾을 수 있다. (예 TagStatus 로딩 시) 2010.2.18 수정
		public EnumTagType type;
        public TagPublicClass ptr;
	}
	...
	public class TagLib
	{
        ...
        public static bool bNeedFileSaveList = true;   // 파일저장 목록을 만들어야 한다.
        ...
        /// <summary>
        /// 태그편집기 등에서 태그를 추가하거나 삭제, 이름변경 등의 변화가 있을 때 TAG_NOT_FOUND를 바꾸어야 태그 없음에서 다시 태그를 찾는다.
        /// </summary>
        ...
        public static void ReMakeTagList()
        {
            ...
            bNeedFileSaveList = true;   // 파일저장 목록을 만들어야 한다.
        }
		...
		static bool RecurseGetTagPosPublic(TagGrClass gr, string tag, ref int[] pos, int depth)
		{
			...
			for(int i = 0; i < gr.arrayTag.Count; i++) 
			{
				...
				if(String.Compare(tp.name, group_name, true) == 0) 
				{
					if(index == -1) 
					{
			...
			for(int i = 0; i < gr.arrayTag.Count; i++) 
			{
				...
				if(String.Compare(tp.name, group_name, true) == 0) 
				{
					...
					else 
					{
						...
						if(String.Compare(member_name, ".value", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_curr;
							if(tp.enumTagType == EnumTagType.ST)
								var_type = EnumTagMemberVar.MEMBER_VAR_string;
						...
						else if(String.Compare(member_name, ".tag", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_tag;
							var_type = EnumTagMemberVar.MEMBER_VAR_string;
						}
						else if(String.Compare(member_name, ".name", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_name;
							var_type = EnumTagMemberVar.MEMBER_VAR_string;
						}
						else if(String.Compare(member_name, ".des", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_description;
							var_type = EnumTagMemberVar.MEMBER_VAR_string;
						}
						else if(String.Compare(member_name, ".unit", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_unit;
							var_type = EnumTagMemberVar.MEMBER_VAR_string;
						}
						else if(String.Compare(member_name, ".desON", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_desON;
							var_type = EnumTagMemberVar.MEMBER_VAR_string;
						}
						else if(String.Compare(member_name, ".desOFF", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_desOFF;
							var_type = EnumTagMemberVar.MEMBER_VAR_string;
						}
						else if(String.Compare(member_name, ".port", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_port;
							var_type = EnumTagMemberVar.MEMBER_VAR_int;
						}
						else if(String.Compare(member_name, ".station", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_station;
							var_type = EnumTagMemberVar.MEMBER_VAR_int;
						}
						else if(String.Compare(member_name, ".address", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_address;
							var_type = EnumTagMemberVar.MEMBER_VAR_int;
						}
						else if(String.Compare(member_name, ".extra1", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_extra1;
							var_type = EnumTagMemberVar.MEMBER_VAR_string;
						}
						else if(String.Compare(member_name, ".extra2", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_extra2;
							var_type = EnumTagMemberVar.MEMBER_VAR_int;
						}
						else if(String.Compare(member_name, ".hihi", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_hihi;
							var_type = EnumTagMemberVar.MEMBER_VAR_float;
						}
						else if(String.Compare(member_name, ".high", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_high;
							var_type = EnumTagMemberVar.MEMBER_VAR_float;
						}
						else if(String.Compare(member_name, ".low", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_low;
							var_type = EnumTagMemberVar.MEMBER_VAR_float;
						}
						else if(String.Compare(member_name, ".lolo", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_lolo;
							var_type = EnumTagMemberVar.MEMBER_VAR_float;
						}
						else if(String.Compare(member_name, ".active", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_act;
							var_type = EnumTagMemberVar.MEMBER_VAR_int;
						}
						else if(String.Compare(member_name, ".full", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_full;
							var_type = EnumTagMemberVar.MEMBER_VAR_float;
						}
						else if(String.Compare(member_name, ".base", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_base;
							var_type = EnumTagMemberVar.MEMBER_VAR_float;
						}
						else if(String.Compare(member_name, ".plcfull", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_plc_full;
							var_type = EnumTagMemberVar.MEMBER_VAR_float;
						}
						else if(String.Compare(member_name, ".plcbase", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_plc_base;
							var_type = EnumTagMemberVar.MEMBER_VAR_float;
						}
						else if(String.Compare(member_name, ".viewfull", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_viewfull;
							var_type = EnumTagMemberVar.MEMBER_VAR_float;
						}
						else if(String.Compare(member_name, ".viewbase", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_viewbase;
							var_type = EnumTagMemberVar.MEMBER_VAR_float;
						}
						else if(String.Compare(member_name, ".assign", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_assign;
							var_type = EnumTagMemberVar.MEMBER_VAR_string;
						}
						else if(String.Compare(member_name, ".NeedAlarmConfirm", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_NeedAlarmConfirm;
							var_type = EnumTagMemberVar.MEMBER_VAR_int;
						}
						else if(String.Compare(member_name, ".protectscan", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_ProtectScan;
							var_type = EnumTagMemberVar.MEMBER_VAR_int;
						}
						else if(String.Compare(member_name, ".protectcontrol", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_ProtectControl;
							var_type = EnumTagMemberVar.MEMBER_VAR_int;
						}
						else if(String.Compare(member_name, ".ProtectAlarmEvent", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_ProtectAlarmEvent;
							var_type = EnumTagMemberVar.MEMBER_VAR_int;
						}
						else if(String.Compare(member_name, ".ProtectAlarmData", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_ProtectAlarmData;
							var_type = EnumTagMemberVar.MEMBER_VAR_int;
						}
						else if(String.Compare(member_name, ".SumTotal", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_fSumTotal;
							var_type = EnumTagMemberVar.MEMBER_VAR_double;
						}
						else if(String.Compare(member_name, ".SumPart", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_fSumPart;
							var_type = EnumTagMemberVar.MEMBER_VAR_double;
						}
						else if(String.Compare(member_name, ".AlarmLevelStatus", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_cAlarmLevelStatus;
							var_type = EnumTagMemberVar.MEMBER_VAR_int;
						}
						else if(String.Compare(member_name, ".Format", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_fDisplayFormat;
							var_type = EnumTagMemberVar.MEMBER_VAR_float;
						}
						else if(String.Compare(member_name, ".alarm", true) == 0) 
						{
							member = EnumTagMember.TAG_MEMBER_alarm;
							var_type = EnumTagMemberVar.MEMBER_VAR_int;
						}
                        else if (String.Compare(member_name, ".DeviceQuality", true) == 0)
                        {
                            member = EnumTagMember.TAG_MEMBER_DeviceQuality;
                            var_type = EnumTagMemberVar.MEMBER_VAR_int;
                        }
                        else if (String.Compare(member_name, ".DeviceSubStatus", true) == 0)
                        {
                            member = EnumTagMember.TAG_MEMBER_DeviceSubStatus;
                            var_type = EnumTagMemberVar.MEMBER_VAR_int;
                        }
                        else if (String.Compare(member_name, ".DeviceLimit", true) == 0)
                        {
                            member = EnumTagMember.TAG_MEMBER_DeviceLimit;
                            var_type = EnumTagMemberVar.MEMBER_VAR_int;
                        }
                        else if (String.Compare(member_name, ".FileSave", true) == 0)
                        {
                            member = EnumTagMember.TAG_MEMBER_bFileSave;
                            var_type = EnumTagMemberVar.MEMBER_VAR_int;
                        }
                        else if (String.Compare(member_name, ".RelayOnDelay", true) == 0)
                        {
                            member = EnumTagMember.TAG_MEMBER_RelayOnDelay;
                            var_type = EnumTagMemberVar.MEMBER_VAR_int;
                        }
                        else if (String.Compare(member_name, ".RelayOffDelay", true) == 0)
                        {
                            member = EnumTagMember.TAG_MEMBER_RelayOffDelay;
                            var_type = EnumTagMemberVar.MEMBER_VAR_int;
                        }
                        else if (String.Compare(member_name, ".RelayPulseTime", true) == 0)
                        {
                            member = EnumTagMember.TAG_MEMBER_RelayPulseTime;
                            var_type = EnumTagMemberVar.MEMBER_VAR_int;
                        }
						else 
						{
        ...
        public static TagAiClass GetStructAI(TagListStruct tls)
        {
            TagPublicClass tp = GetStructPublic(tls);
            if (tp.enumTagType == EnumTagType.AI)
            {
                return (TagAiClass)tp;
            }
        ...
        public static TagAoClass GetStructAO(TagListStruct tls)
        {
            TagPublicClass tp = GetStructPublic(tls);
            if (tp.enumTagType == EnumTagType.AO)
            {
        ...
        public static TagDiClass GetStructDI(TagListStruct tls)
        {
            TagPublicClass tp = GetStructPublic(tls);
            if (tp.enumTagType == EnumTagType.DI)
            {
        ...
        public static TagDoClass GetStructDO(TagListStruct tls)
        {
            TagPublicClass tp = GetStructPublic(tls);
            if (tp.enumTagType == EnumTagType.DO)
            {
        ...
        public static TagStClass GetStructST(TagListStruct tls)
        {
            TagPublicClass tp = GetStructPublic(tls);
            if (tp.enumTagType == EnumTagType.ST)
            {
        ...
        // 포인트를 보관해서 하는것이 속도가 두배정도 빠르다.  2010.2.18
        public static TagPublicClass GetStructPublic(TagListStruct tls)
        {
            /*
            if (tls.ptr == null)
            {
                tls.ptr = GetStructPublic(tls.tag, ref tls.tag_pos);
            }*/
		...
		static void RecurseGetTagList(TagGrClass gr, ArrayList array, EnumTagType type, int depth, ref int[] pos)
		{
			TagPublicClass tp;
			...
			for(int i = 0; i < gr.arrayTag.Count; i++) 
			{
				...
				else if(tp.enumTagType == type) 
				{
					...
					list.tag = tp.tag;
					list.tag_pos_save = new int[depth+1];
					list.type = tp.enumTagType;
                    list.ptr = tp;
					Array.Copy(pos, list.tag_pos_save, depth+1);
        ...
        /// <summary>
        /// MakeTag리스트와는 다르게 특정 종류의 태그만 가져온다.
        /// </summary>
		...
		public static TagListStruct[] GetTagList(EnumTagType type)
		{
            ...
            //if(array.Count == 0)	return null; null 보다는 [0]으로 할당해서 Length가 0이 되도록 하는 것이 다운을 막는데 유리하다. 9.5.2
		...
		static void RecurseMakeTagList(TagGrClass gr, ArrayList array, int depth, ref int[] pos)
		{
			TagPublicClass tp;
		...
		public static void RecurseGroupTree(TreeNode tree, ArrayList array)
		{
			TagPublicClass pub;
			for(int i = 0; i < array.Count; i++) 
			{
        ...
        public static TagStClass GetDirectTag(TagStClass st)
        {
            if (st.cTagLinkType != 3) return st;					// 간접태그가 아니면

            if (st.assign == null) return st;
            if (st.assign.pos[0] == TAG_NOT_FOUND) return st;	// assign 되어 있지 않다.

            return GetStructST(st.assign.tag, ref st.assign.pos);
        }

        public static TagPublicClass GetDirectTag(TagPublicClass tp)
        {
            if (tp.cTagLinkType != 3) return tp;					// 간접태그가 아니면

            if (tp.assign == null) return tp;
            if (tp.assign.pos[0] == TAG_NOT_FOUND) return tp;	// assign 되어 있지 않다.

            return GetStructPublic(tp.assign.tag, ref tp.assign.pos);
        }
