...
namespace AutoLibLocal
{
	...
	public class TagUtil
	{
		...
		static public string AiValueToString(TagAiClass ai, double value)
		{
			...
			if(ai.cDisplayFormat == '{') 
			{
				...
				if(ai.cDisplayFormat == 'E')                         
					format_string = "{0,"+high.ToString()+":E"+low.ToString()+"}";
				else if(ai.cDisplayFormat == 'e')
					format_string = "{0,"+high.ToString()+":e"+low.ToString()+"}";
				else
					format_string = "{0,"+high.ToString()+":F"+low.ToString()+"}";
		...
		static public double GetDisplayValue(TagAiClass ai, double val)
		{
			...
			{
				double real_gab = ai.fFull-ai.fBase;
                double view_gab = Math.Abs(ai.view_full);
                ...
                if (val < 0)
                {
                    if (view_gab == 0)
                        ...
                        val = (real_gab * Math.Abs(val) / view_gab);
		...
		static string[] sTagMember = {  
                                         ...
                                         "DeviceQuality", "DeviceSubStatus", "DeviceLimit",
									 };
		...
		public static bool IsTagMember(string member)
		{
			...
			{
				if(String.Compare(member, sTagMember[i], true) == 0) 
				{
					return true;
				}
			}
        ...
        public static string ApplyDeviceQuality(TagPublicClass tp, string curr)
        {
            if (!ConfigViewMain.bUseDeviceQuality) return curr;

            if (tp.DeviceQuality == EnumDeviceQuality.Good) return curr;

            string des;

            des = String.Format("{0}({1},{2})", curr, tp.DeviceQuality, tp.DeviceSubStatus);

            return des;
        }

        public static string AiValueToStringWithDeviceQuality(TagAiClass ai)
        {
            return ApplyDeviceQuality(ai, AiValueToString(ai, ai.curr));
        }
        ...
        static void AddTagUsedInformation(MULTI_SELECT_TAG_STRUCT list, string zone, EnumTagUsedType type, object obj, string position)
        {
            ...
            {
                list.arrayUsed = new ArrayList();
            }

            TAG_USED_INFOMATION used = new TAG_USED_INFOMATION();

            if (String.Compare(TotalConfig.sDirWorkProject, 0, zone, 0, TotalConfig.sDirWorkProject.Length, true) == 0)
            {
        ...
        public static void AddTagList(ArrayList block, string tag, EnumTagType tag_type, string zone, EnumTagUsedType type, object obj, string position)
        {
            if (tag == null) return;
    ...
    public class MULTI_SELECT_TAG_STRUCT
    {
        ...
        public ArrayList arrayUsed = null;  // 사용된 곳
    }
}
