...
namespace LocalMain.OPCUA
{
    ...
    internal static class OpcUaClientCache
    {
        ...
        public static void Update(TagValueChangedEvent e)
        {
            ...
            // =====================================================
            //  Warmup(ReadOnce) 완료 처리
            // =====================================================
            lock (_sync)
            {
                _warmupPending.Remove(e.FullName);
            }
        }
        ...
        // =========================================================
        // Warmup(ReadOnce) 요청 관리
        // =========================================================
        private static readonly HashSet<string> _warmupPending =
            new HashSet<string>(StringComparer.OrdinalIgnoreCase);

        /// <summary>
        /// Cache miss 시 ReadOnce 워밍업 요청 (중복 방지)
        /// </summary>
        public static bool TryRequestWarmup(string fullName)
        {
            if (string.IsNullOrEmpty(fullName))
                return false;
            ...
            lock (_sync)
            {
                if (_warmupPending.Contains(fullName))
                    return false;

                _warmupPending.Add(fullName);
                return true;
            }
        }
