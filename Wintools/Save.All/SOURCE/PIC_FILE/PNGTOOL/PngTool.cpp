...
pngClass :: pngClass() : pictureFileClass()
{
...
int pngClass :: ReadOpen(HWND hwnd, const char *filename)
{
	BITMAPFILEHEADER *bf;
	BITMAPINFOHEADER *bi;
	PNGHEADER header;
	...
	fread(&header, 1, sizeof(PNGHEADER), in);
	...
	if(header.signature[0] != 137 ||
		...
		header.signature[7] != 10) {	// 137 80 78 71 13 10 26 10
		MessageBox(NULL, "PNG 파일이 아닙니다.", "파일 형식 틀림", MB_OK);
		fclose(in);
		delete bf;
		delete bi;
		return 0;
	...
	while(1) {
		if(fread(&chunk.Length, 1, 8, in) != 8)	break;
		chunk.Length = MOTO2IBM(chunk.Length);
		//fseek(in, chunk.Length, SEEK_CUR);
		//fread(&chunk.CRC, 1, 4, in);

		if(strncmp((char*)chunk.ChunkType, "IHDR", 4) == 0) {
			fread(&chunkIHDR, 1, sizeof(CHUNK_IHDR), in);
			chunkIHDR.Width = MOTO2IBM(chunkIHDR.Width);
			chunkIHDR.Height = MOTO2IBM(chunkIHDR.Height);
		}
		else if(strncmp((char*)chunk.ChunkType, "pHYs", 4) == 0) {
			CHUNK_PHYS phys;
			fread(&phys, 1, sizeof(CHUNK_PHYS), in);
			phys.PixelsPerUnitX = MOTO2IBM(phys.PixelsPerUnitX);
			phys.PixelsPerUnitY = MOTO2IBM(phys.PixelsPerUnitY);
		}
		else if(strncmp((char*)chunk.ChunkType, "IDAT", 4) == 0) {
			fseek(in, chunk.Length, SEEK_CUR);
		}
		else if(strncmp((char*)chunk.ChunkType, "IEND", 4) == 0) {
			break;
		}
		else {
			fseek(in, chunk.Length, SEEK_CUR);
...
int pngClass :: GetOneLineCompression0(BYTE *buf, int /*maxx*/)
{
...
int pngClass :: GetOneLineCompressionRLE8(BYTE *buf, int /*maxx*/)
{
	int ch1, ch2, x = 0, i;
...
int pngClass :: GetOneLine(BYTE *buf, int maxx)
{
	switch(compression) {
...
int pngClass :: PutOneLine(BYTE *buf)
{
...
void pngClass :: MakeHeader(BITMAPFILEHEADER *bf, BITMAPINFOHEADER *bi)
{
	int  palsize = 0;
	DWORD datasize;
...
int pngClass :: WriteOpen(const char *filename)
{
