...
HPICTURE PictureLoadTif(HWND hwnd, char *filename)
{
	...
	if(!tif.ReadOpen(filename)) {
		return NULL;
	}
	tif.GetDac(dac.data);
	tif.GetSize(width, height);

	widthbyte = (WORD)BmpWidthToByte(width, tif.GetColor());
	...
	hPic = PictureLoadNew(hwnd, width, height, tif.GetColor(), dac.data);
	...
	for(i = 0; i < height; i++) {
		WaitCursorStatus(i);
		if(!tif.GetOneLine(buf, width))	break;
