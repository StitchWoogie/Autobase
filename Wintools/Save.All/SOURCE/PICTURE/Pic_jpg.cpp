...
static PICTURE_STRUCT *picJpeg;
static int nCurrentX, nCurrentY;
...
/* Initialize for input; return image size and component data. */
{
   ...
   */
  cinfo->image_width = picJpeg->back.nWidth;			/* width in pixels */
  cinfo->image_height = picJpeg->back.nHeight;		/* height in pixels */
...
/* Read next row of pixels into pixel_row[][] */
{
  ...
  register long col;
  StackBYTE stack(60000u);

  ptr0 = pixel_row[0];
  ptr1 = pixel_row[1];
  ptr2 = pixel_row[2];

  PictureGetOneLine(picJpeg, nCurrentY, stack.data);
  WaitCursorStatus(nCurrentY);
  for (col = 0; col < cinfo->image_width; col++) {
    *ptr0++ = (JSAMPLE) stack.data[col*3+2]; /* red */
    *ptr1++ = (JSAMPLE) stack.data[col*3+1]; /* green */
    *ptr2++ = (JSAMPLE) stack.data[col*3+0]; /* blue */
  }
  nCurrentY++;
}
...
int PictureSaveJpg(HWND hwnd, HPICTURE hPic, char *filename)
{
	...
	*/
	...
	nCurrentY = 0;
	picJpeg = (PICTURE_STRUCT *) PictureLock(hPic);

	if(picJpeg->back.nBitsPerPixel != COLOR_1600) {
		PictureUnlock(hPic, picJpeg);  		
		MessageBox(hwnd, "You can save only 24bit color.", "Jpeg save error", MB_OK);
		return 0;
	}

	if ((cinfo.output_file = fopen(filename, "wb")) == NULL) {
		PictureUnlock(hPic, picJpeg);  			
		MessageBox(hwnd, "Can't open file.", filename, MB_OK);
		return 0;
	}
	
	WaitCursorStart(0, picJpeg->back.nHeight);
	/* Here we go! */
	jpeg_compress(&cinfo);
...
error_exit (const char *msgtext)
{
	...
	sTextError = new char[strlen(stack.data)+1];

	if(sTextError) {
		strcpy(sTextError, stack.data);
	}
...
METHODDEF void
put_pixel_rows_memory(decompress_info_ptr cinfo, int num_rows, JSAMPIMAGE pixel_data)
/* Write some rows of output data */
{
  register JSAMPROW ptr0, ptr1, ptr2;
  register long col;
  register int row;
  StackBYTE gbuf(64000u);

  for (row = 0; row < num_rows; row++, nCurrentY++) {
	 WaitCursorStatus(nCurrentY);
    //gprintf(180, 2, DARK_COLOR, WHITE_COLOR, "%d%", (long)nCurrentY*100/cinfo->image_height);
    if(cinfo->jpeg_color_space == CS_GRAYSCALE) {
       ptr0 = pixel_data[0][row];
       for (col = 0; col < cinfo->image_width; col++) {
			 gbuf.data[(unsigned)col] = (*ptr0);     	// red
			 ptr0++;
       }
    }
    else {
       ...
       for (col = 0; col < cinfo->image_width; col++) {
			 gbuf.data[(unsigned)col*3+2] = (*ptr0);     	// red
			 gbuf.data[(unsigned)col*3+1] = (*ptr1);      	// green
			 gbuf.data[(unsigned)col*3+0] = (*ptr2);  	// blue
			 ptr0++;
			 ptr1++;
			 ptr2++;
       }
    }
	 PicturePutOneLine(picJpeg, nCurrentY, gbuf.data);
    //PicturePutOneLine(picture, nCurrentY, gbuf);
  }
}
...
METHODDEF void
d_ui_method_selection_memory (decompress_info_ptr cinfo)
{
  ...
  cinfo->methods->put_pixel_rows = put_pixel_rows_memory;
  cinfo->methods->output_term = output_term;
}

GLOBAL void jpeg_readheader (decompress_info_ptr cinfo);
GLOBAL void jpeg_decompress_except_header (decompress_info_ptr cinfo);
...
HPICTURE PictureLoadJpg(HWND hwnd, char *filename)
{
   ...
   if ((cinfo.input_file = fopen(filename, "rb")) == NULL) {
	  MessageBox(hwnd, "파일을 열 수 없습니다.", filename, MB_OK);
     return NULL;
   }
   ...
   if (setjmp(setjmp_buffer)) {
      ...
      */
		...
		MessageBox(hwnd, "파일을 읽는 도중 오류가 발생했습니다.", sTextError, MB_OK);
		return 0;
   }
    ...
    * image type.
    */
   dc_methods.d_ui_method_selection = d_ui_method_selection_memory;
   ...
   jpeg_readheader(&cinfo);

   sizex = (int)cinfo.image_width;
   sizey = (int)cinfo.image_height;
   ...
   if(cinfo.jpeg_color_space == CS_GRAYSCALE) {
	...
	hPic = PictureLoadNew(hwnd, sizex, sizey, color, dac.data);
	...
	picJpeg = (PICTURE_STRUCT *) PictureLock(hPic);
	WaitCursorStart(0, sizey);
   //gbufget(0, 0, MAXX, 20);
   //gcls(0, 0, MAXX, 20, 1, WHITE_COLOR);
   //gput(10, 2, "그림을 읽고 있습니다:", 2, WHITE_COLOR);

   nCurrentY = 0;

   jpeg_decompress_except_header(&cinfo);
	WaitCursorEnd();
	PictureUnlock(hPic, picJpeg);

   //gbufput(0, 0);
   //memcpy(picture->dac, dac, 768);
