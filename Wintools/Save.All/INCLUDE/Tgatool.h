...
typedef	struct {
	...
	BYTE	cmtype;		// color map type (0 for none)
	BYTE	imagetype;      // image type (1 is color map image)
				// image type (2 is unmapped RGB)
				// image type (3 is B/W image)
	...
	WORD	xsize;
	WORD	ysize;
	BYTE	bitsperpixel;	// number of bits per pixel in pict
	BYTE	imagedesc;	// 0 for TGA and TARGA images
} TGAHEADER;

// 헤더 다음에 오는 한바이트의 값을 읽어서 값이 0이 아니면 다음에
// 길이만큼 이미지 식별을 위한 정보가 들어있다.
//
...
class tgaClass : public pictureFileClass {
		TGAHEADER head;
		int image_type;		//  0  -  No image data included.
		...
		int  nRemainCount;				// 다음 라인까지 압축이 계속될 때 압축을 덜푼 갯수.
		char bRemainSameOrDifferent;	// 계속 이어지는 압축이 같은 count냐 다른 count냐?
		WORD wRemainWord;					// 계속 이어지는 압축에서 같은 count일때 그 값(32768색상에서 사용)

		int GetOneLineType10(BYTE *buf, int limitx);
		int GetOneLineType10_32768(BYTE *buf, int limitx);
		int GetOneLineType10_1600(BYTE *buf, int limitx);
		int GetOneLineType10_32bit(BYTE *buf, int limitx);
		void PutRGB(TGAHEADER *head, BYTE *dac);
		void MakeHeader(TGAHEADER *head);
	public:
		tgaClass(void);
		~tgaClass(void);
