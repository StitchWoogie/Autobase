...
{
    public class HashString : HashToolRoot
    {
        ...
        public string Encode(string source, string seed)
        {
            if (!IsNormalAccess()) return "";
        ...
        // 64개의 데이터이므로 각 글자마다 6Bit가 나오므로 적어도 글자 하나는 3Byte가 할당되어야 한다.
        public string Decode(string source, string seed)
        {
            if (!IsNormalAccess()) return "";
        ...
        // 20 바이트짜리 Seed를 만든다.
        public static string MakeRandomSeedString()
        {
